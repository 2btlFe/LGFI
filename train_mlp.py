...
import argparse
from loss.loss import Main_Loss
...
if __name__ == "__main__":
    ...
    parser.add_argument('--main_loss2', type=str, default='triplet')
    ...
    mtcnn = MTCNN(
        ...
        keep_all=True
        # device=device mtcnn에서는 gpu 사용이 안된대요
    )
    ...
    # Create an optimizer
    optimizer = optim.Adam(params=model.parameters(), lr=learning_rate_2)
    ...
    work_dir = f'work_dir/{args.model_name}_{formatted_time}'
    writer = SummaryWriter(work_dir)
    # Loss
    main1_loss = Main_Loss('ce').get_loss_function()
    # main2_loss = Main_Loss(args.main_loss, criterion=criterion).get_loss_function() # 쓰실거면 주석해제해주세요
    ...
    for epoch in range(0, ce_epochs):
        ...
        if epoch % save_period ==0:            
            save_path = f'{work_dir}/{args.model_name}_{num_epochs}_{batch_size}_deepfake_{epoch}.pth'
