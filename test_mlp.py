...
from model.InceptionResnetV1_with_MLP import InceptionResnetV1_with_MLP
...
def _trans_lambda(img):
    return np.array(img, dtype=np.float32)
def _test_trans_lambda(img):
    return np.array(img, dtype=np.float32)
trans = transforms.Compose([
    transforms.Lambda(_trans_lambda),  # PIL 이미지를 float32로 변환
    transforms.ToTensor(),  # 이미지를 텐서로 변환
    fixed_image_standardization  # 고정 이미지 표준화 적용
])

test_trans = transforms.Compose([
    transforms.Lambda(_test_trans_lambda),  # PIL 이미지를 float32로 변환
    transforms.ToTensor(),  # 이미지를 텐서로 변환
    fixed_image_standardization  # 고정 이미지 표준화 적용
])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--trained_model', type=str, default='model~~')
    ...
    logger.info(f'pretrained : {args.trained_model}')
    ...
    model = InceptionResnetV1_with_MLP(num_classes=60).to(device)
    ...
    for image, label in test_loader:
        ...
        output = model(image)
        _, predictions = torch.sort(output, descending=True)
        top5_labels=predictions[0][0:5]
        
        # print(top5_labels)
        # print(label)
        # print('----------')
        total += 1
        if int(label) in top5_labels:
            correct += 1
            correct_classes.append(label)
        else:
            incorrect_classes.append(label)
