...
reader = imageio.get_reader('driving_videos/blink.mp4')
...
generator, kp_detector = load_checkpoints(
    ...
    checkpoint_path='first_order_model/config/vox-cpk.pth.tar',
    cpu=True
)

def apply_first_order_model(image, generator, kp_detector, driving_video):
    predictions = make_animation(
        skimage.transform.resize(np.asarray(image), (256, 256)),
        driving_video,
        generator,
        kp_detector,
        relative=True,
    ...
    return predictions[25]  # Return the first frame of the generated video for simplicity

# Transform 
trans = transforms.Compose([
    transforms.Lambda(lambda img: apply_first_order_model(img, generator, kp_detector, driving_video)),
    transforms.Lambda(lambda img: np.array(img, dtype=np.float32)),  # PIL 이미지를 float32로 변환
    transforms.ToTensor(),  # 이미지를 텐서로 변환
    fixed_image_standardization  # 고정 이미지 표준화 적용
])
